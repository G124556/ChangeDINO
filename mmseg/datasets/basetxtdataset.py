...
class TXTCDDatasetJSON(BaseDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if osp.isfile(self.ann_file):
            lines = mmengine.list_from_file(
                self.ann_file, backend_args=self.backend_args)
            if os.path.exists(os.path.dirname(lines[0].strip().split('  ')[0])+'_dinocls_56_vit16.json'):
                jsondataA = self.load_json(os.path.dirname(lines[0].strip().split('  ')[0])+'_dinocls_56_vit16.json')
                jsondataB = self.load_json(os.path.dirname(lines[0].strip().split('  ')[1])+'_dinocls_56_vit16.json')
            for line in lines:
                pimgA = line.strip().split('  ')[0]
                pimgB = line.strip().split('  ')[1]
                plab = line.strip().split('  ')[2]
