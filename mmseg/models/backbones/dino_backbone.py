...
#     def __init__.py(self, spacial_dim: int, embed_dim: int, num_heads: int, output_dim: int = None):
#         super().__init__.py()
...
#
#         # If the input channel dimension doesn't match the embed_dim, use a projection
#         if C != target_dim:
#             proj = nn.Conv2d(C, target_dim, kernel_size=1).to(x.device)
#             x = proj(x)
#
#         x = x.reshape(x.shape[0], target_dim, x.shape[2] * x.shape[3]).permute(2, 0, 1)  # NCHW -> (HW)NC
#         x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (HW+1)NC
#
#         cls_pos = self.positional_embedding[0:1, :]
#         spatial_pos = F.interpolate(self.positional_embedding[1:,].reshape(1, self.spacial_dim, self.spacial_dim, target_dim).permute(0, 3, 1, 2), size=(H, W), mode='bilinear')
#         spatial_pos = spatial_pos.reshape(target_dim, H*W).permute(1, 0)
...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2] * x.shape[3]).permute(2, 0, 1)  # NCHW -> (HW)NC x:(64.1,2048)

        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (HW+1)NC  x:(65.1,2048)

        cls_pos = self.positional_embedding[0:1, :]  # cls_pos (1,2048)
        spatial_pos = F.interpolate(self.positional_embedding[1:,].reshape(1, self.spacial_dim, self.spacial_dim, self.embed_dim).permute(0, 3, 1, 2), size=(H, W), mode='bilinear')  #(1,8,8,2048)
        spatial_pos = spatial_pos.reshape(self.embed_dim, H*W).permute(1, 0)  #(64,2048)
        positional_embedding = torch.cat([cls_pos, spatial_pos], dim=0)   #(64,2048)
...
class DINOResNet(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            print(u, w, 'are misaligned params in DINOResNet')
...
@MODELS.register_module()
class DINOResNetWithAttention1(nn.Module):
    def __init__(self, layers, output_dim=1024, input_resolution=224, width=64, pretrained=True,
                 model_variant='resnet50', **kwargs):
        super().__init__()
        self.pretrained = pretrained
        self.output_dim = output_dim
        self.input_resolution = input_resolution
        ...
        print("使用的backbone是：", model_variant)

        # Adjust width and block type based on model variant
        if model_variant == 'resnet18' or model_variant == 'resnet34':
            block = BasicBlock
            width = 64  # ResNet18 and ResNet34 use 64 as base width
        else:  # ResNet50
            block = Bottleneck
            width = 64  # ResNet50 uses 64 as base width, but expands it in the Bottleneck
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            # 直接使用对应模型的预训练权重
            if self.model_variant in ['resnet18', 'resnet34', 'resnet50']:
                state_dict = checkpoint
                if 'state_dict' in checkpoint:
                    state_dict = checkpoint['state_dict']
                # 移除可能的'module.'前缀（如果权重来自DataParallel模型）
            ...
            if 'attnpool.positional_embedding' in state_dict:
                if self.attnpool.positional_embedding.shape != state_dict['attnpool.positional_embedding'].shape:
                    print(
                        f'Resize the pos_embed shape from {state_dict["attnpool.positional_embedding"].shape} to {self.attnpool.positional_embedding.shape}')
                    pos_embed = state_dict['attnpool.positional_embedding']
                    cls_pos = pos_embed[0:1, :]
                    H = W = self.input_resolution // 32
                    old_h = int(math.sqrt(pos_embed[1:, ].shape[0]))
                    spatial_pos = F.interpolate(
                        pos_embed[1:, ].reshape(1, old_h, old_h, cls_pos.shape[1]).permute(0, 3, 1, 2),
                        size=(H, W), mode='bilinear'
                    )
                    spatial_pos = spatial_pos.reshape(cls_pos.shape[1], H * W).permute(1, 0)
                    new_pos_embed = torch.cat([cls_pos, spatial_pos], dim=0)
                    state_dict['attnpool.positional_embedding'] = new_pos_embed

            # 加载权重
            u, w = self.load_state_dict(state_dict, strict=False)
            print(f"Unmatched keys: {u}")
            print(f"Unexpected keys: {w}")
    ...
    #         checkpoint = torch.load(pretrained, map_location='cpu')
    #
    #         if self.model_variant in ['resnet18', 'resnet34']:
    #             # Convert ResNet50 weights to ResNet18/34 format if necessary
    #             state_dict = self._convert_resnet50_to_resnet18_34(checkpoint)
    #         else:
    ...
    #                 old_h = int(math.sqrt(pos_embed[1:, ].shape[0]))
    #                 spatial_pos = F.interpolate(
    #                     pos_embed[1:, ].reshape(1, old_h, old_h, cls_pos.shape[1]).permute(0, 3, 1, 2), size=(H, W),
    #                     mode='bilinear')
    #                 spatial_pos = spatial_pos.reshape(cls_pos.shape[1], H * W).permute(1, 0)
    #                 new_pos_embed = torch.cat([cls_pos, spatial_pos], dim=0)
    #                 state_dict['attnpool.positional_embedding'] = new_pos_embed
    ...
    def _convert_resnet50_to_resnet18_34(self, checkpoint):
        new_state_dict = {}
        for k, v in checkpoint.items():
            if k.startswith('visual.'):
                ...
                if 'layer' in k:
                    parts = k.split('.')
                    layer_num, block_num = parts[0], int(parts[1])
                    if block_num >= len(getattr(self, layer_num)):
                        continue  # Skip extra blocks
                    if 'downsample' in k:
                        new_k = k.replace('downsample.0', 'downsample.0').replace('downsample.1', 'downsample.1')
                    else:
                        if 'conv1' in k:
                            new_k = k.replace('conv1', 'conv1')
                        elif 'conv2' in k:
                            new_k = k.replace('conv2', 'conv2')
                        elif 'conv3' in k:
                            continue  # Skip conv3 for ResNet18/34
                        else:
                            new_k = k
                else:
                    new_k = k
...
#     expansion = 4
#
#     def __init__.py(self, inplanes, planes, stride=1, downsample=None):
#         super(Bottleneck, self).__init__.py()
...
# Assuming AttentionPool2d class is defined elsewhere in your code
...
@MODELS.register_module()
class DINOResNetWithAttention(nn.Module):
    """
    A ResNet class that is similar to torchvision's but contains the following changes:
    - There are now 3 "stem" convolutions as opposed to 1, with an average pool instead of a max pool.
    - Performs anti-aliasing strided convolutions, where an avgpool is prepended to convolutions with stride > 1
    ...
    def __init__(self, layers, output_dim=1024, input_resolution=224, width=64, pretrained=True, **kwargs):
        super().__init__()
        self.pretrained = pretrained
        self.output_dim = output_dim
        self.input_resolution = input_resolution
    ...
    def forward(self, x):
        ...
        outs = []
        x = self.layer1(x) #x:(1,256,64,64)
        outs.append(x)
        x = self.layer2(x)
        outs.append(x)     #x:(1,256,64,64)
        x = self.layer3(x)
        outs.append(x)     #x:(1,1024,16,16)
        x = self.layer4(x)
        outs.append(x) # (1,2048,8,8)
...
@MODELS.register_module()
class DINOTextContextEncoder(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        pretrained = pretrained or self.pretrained
        if isinstance(pretrained, str):
            # checkpoint = torch.jit.load(pretrained, map_location='cpu')
            checkpoint = torch.load(pretrained, map_location='cpu')
            state_dict = {}
...
class DINOTextContextEncoder1(nn.Module):
    def __init__(self, context_length=22,
                 vocab_size=49408,
                 transformer_width=512,
                 transformer_heads=8,
    ...
    def forward(self, text, context):
        ...
        # text = text.to(self.linear.weight.dtype)
        # text = self.linear(text.reshape(5, -1)).reshape(5, 2, 64)
        #
        # print(text.dtype)  # 检查输入张量的数据类型
        # print(self.token_embedding.weight.dtype)  #
        # x_text = self.token_embedding(text)     #text.size 5 2 64   ,x_text.size 5 2 64 512  # n_clas, n_text, C

        x_text=text
        batch_size, seq_len, feat_dim = x_text.shape
        x_text = x_text.transpose(1, 2)
        x_text = self.linear(x_text)
        x_text = x_text.reshape(batch_size, 2, 64, 512)


        N, K, N1, C = x_text.shape   #5  2 64 512
        N, B, N2, C = context.shape  #5 1 13 512

        # eos_indx = text.argmax(dim=-1) + N2
        eos_indx = x_text.argmax(dim=-1).argmax(dim=-1) + N2
        eos_indx = eos_indx.reshape(N, K).unsqueeze(1).reshape(-1)

        # x_text = x_text.reshape(1, K, N1, C).expand(B, K, N1, C)
        context = context.expand(N, K, N2, C)  # 5 2 13 512

        x = torch.cat([x_text[:,:,0:1], context, x_text[:, :, 1:]], dim=2).reshape(N*K, N1+N2, C) #10 77 512

        x = x + self.positional_embedding
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)  # 77 10 512
        x = x.permute(1, 0, 2)   # LND -> NLD
        x = self.ln_final(x)
        x = x[torch.arange(x.shape[0]), eos_indx] @ self.text_projection # self.text_projection: 512 1024
        x = x.reshape(N, K, self.embed_dim) #5 2  1024
        return x
