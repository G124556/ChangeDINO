...
def dino_infer(image_pilA, image_pilB, model, preprocess):
    ...
    text = dino.tokenize(class_names).to(device)
...
def init_dino():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model, preprocess = dino.load('ViT-B/16', device=device)
    return model, preprocess



if __name__ == '__main__':
    dino_infer('/home/user/dsj_code/mmsegcd/ChangeDINO/demo/demo.png')