...
def dino_big_image(image_path, dino_save_dir, args, to_label=False):
    ...
    Given fixed dino size and stride size to generate
    dinoped image, the intersection　of width and height is determined.
    For example, given one 6800 x 7200 original image, the dino size is
    ...
    for count, box in enumerate(boxes):
        ...
        if count % 3 == 0:
            mmcv.imwrite(
                ...
                osp.join(
                    dino_save_dir.replace('train', 'val'),
...
def main():
    ...
    for i, img_path in enumerate(src_path_list):
        ...
        dino_big_image(img_path, dst_img_dir, args, to_label=False)
        dino_big_image(label_path, dst_label_dir, args, to_label=True)
        prog_bar.update()
