...
def test_CLAHE():
    # test assertion if dino_limit is None
    with pytest.raises(AssertionError):
        transform = dict(type='CLAHE', dino_limit=None)
        TRANSFORMS.build(transform)
    ...
    transform = dict(type='CLAHE', dino_limit=2)
    transform = TRANSFORMS.build(transform)
    results = dict()
    img = mmcv.imread(
        osp.join(osp.dirname(__file__), '../data/color.jpg'), 'color')
    ...
    assert np.allclose(results['img'], converted_img)
    assert str(transform) == f'CLAHE(dino_limit={2}, tile_grid_size={(8, 8)})'
