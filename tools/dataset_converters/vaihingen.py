...
def dino_big_image(image_path, dino_save_dir, to_label=False):
    # Original image of Vaihingen dataset is very large, thus pre-processing
    # of them is adopted. Given fixed dino size and stride size to generate
    # dinoped image, the intersection　of width and height is determined.
    # For example, given one 5120 x 5120 original image, the dino size is
    ...
    for box in boxes:
        ...
        dinoped_image = image[start_y:end_y,
        ...
        for zipp in zipp_list:
            ...
            for i, src_path in enumerate(src_path_list):
                ...
                if 'noBoundary' in src_path:
                    ...
                    dino_big_image(src_path, dst_dir, to_label=True)
                else:
                    dst_dir = osp.join(out_dir, 'img_dir', data_type)
                    dino_big_image(src_path, dst_dir, to_label=False)
                prog_bar.update()
